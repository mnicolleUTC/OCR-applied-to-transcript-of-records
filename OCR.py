...
def convert_pdf(pdf_path, dpi_val, save_image=False, save_raw_text=False):
    ...
    dpi_val : int
        ...
        PDF, higher is better but anything above 300 is usually not discernible
        to the naked eye. Recommended value ensuring OCR performance = 600
    ...
    pages = convert_from_path(pdf_path, dpi_val)
    list_pages = list()
    student_name = ""
...
def identify_student_name(raw_text_data):
    ...
    split_line_name = re.split(r',|\s', line_name)
...
def identify_erasmus_semester(page):
    ...
    # Identify erasmus destination and credits
    ...
    split_erasmus = re.split(r',|\s', erasmus_information)
...
def identify_education_period(list_index, list_text_extract):
    """
...
def identify_same_semester_type(semester_type_candidate, list_text_extract):
    ...
    education_period = identify_education_period(index, list_text_extract)
    return education_period
...
def identify_student_information(first_page):
    ...
    list_candidate = semester_auto + semester_prin
    # Sorting of candidate based on their fuzzywuzzy score
    list_candidate.sort(key=lambda x: x[1], reverse=True)
    # Selection of the two best matches
    match = [list_candidate[0][0], list_candidate[1][0]]
    ...
    if test_auto[0][1] and test_auto[1][1] > ratio_id:
        ...
        # Case with different semester types
        index = [
            split_line.index(test_prin[0][0]),
            split_line.index(test_auto[0][0]),
        ]
        education_period = identify_education_period(index, split_line)
...
def identify_student_speciality(split_text):
    ...
    # Text treatments
    index_number = line_spec.find(re.findall('[0-9]', line_spec)[-1])
    line_spec = line_spec[0:index_number]
    ...
    list_filtered = split_line[index + 1:]
    speciality = ' '.join(list_filtered)
...
def clean_subject_code(line):
    ...
    """
    # Dictionary giving common corrections for the 2 first letters of subject
    # code
    clean_letter = {
        '0': 'O',
        'o': 'O',
    ...
    # Dictionary giving common corrections for the 2 last digits of subject
    # code
    clean_number = {
        'O': '0',
        'o': '0',
...
def identify_subject_code(subject_pattern, line, switch=False):
    """
...
def extract_data(file_path):
    ...
    # Initialize variable
    credit_number, period, speciality, erasmus_credits, erasmus_country = \
        [0, "", "", 0, ""]
    ...
    for i, page in enumerate(list_pages):
        ...
        if i == 0:
            credit_number, period, speciality = identify_student_information(
                page)
